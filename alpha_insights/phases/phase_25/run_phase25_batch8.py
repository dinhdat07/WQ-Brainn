...
def simulate(expression, decay_val, trunc_val):
    data = {
        "type": "REGULAR",
        "settings": {
    ...
    while True:
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"Spawned: {sim_id} for D={decay_val}, T={trunc_val}")
                return sim_id
...
base_expr = "trade_when(volume > adv20, group_rank(ts_rank(ts_regression(sales, ts_step(1), 252, rettype=2), 60), sector) + group_rank(ts_rank(scl12_sentiment_fast_d1, 20), sector) + group_rank(ts_rank(-mean_corporate_action_sentiment, 20), sector), -1)"
...
for decay_val, trunc_val in runs:
    expr = f"ts_decay_linear({base_expr}, {decay_val})"
