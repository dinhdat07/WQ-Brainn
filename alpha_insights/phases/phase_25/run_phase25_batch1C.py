...
exprs = [
    # 1. Social Sentiment Matrix
    "ts_decay_linear(group_zscore(scl12_sentiment_fast_d1, subindustry), 5)",
    
    # 2. Social Buzz Matrix (Inverted)
    "ts_decay_linear(group_zscore(-scl12_buzz_fast_d1, subindustry), 5)",
    
    # 3. Corporate Action Sentiment (Inverted based on previous run)
    "ts_decay_linear(group_zscore(-mean_corporate_action_sentiment, subindustry), 5)",
    
    # 4. F-Score Quality
    "ts_decay_linear(group_zscore(fscore_bfl_quality, subindustry), 5)",
    
    # 5. Long-term Sales Trend (Regression slope)
    "ts_decay_linear(group_zscore(ts_regression(sales, ts_step(1), 252, rettype=2), subindustry), 10)",
    
    # 6. Long-term Cash Trend (Regression slope)
    "ts_decay_linear(group_zscore(ts_regression(cash, ts_step(1), 252, rettype=2), subindustry), 10)"
]
