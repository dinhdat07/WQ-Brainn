...
base_sales = "group_rank(ts_rank(ts_regression(sales, ts_step(1), 252, rettype=2), 60), sector)"
base_soc = "group_rank(ts_rank(scl12_sentiment_fast_d1, 20), sector)"
base_corp = "group_rank(ts_rank(-mean_corporate_action_sentiment, 20), sector)"

exprs = [
    # 1. Weight 2, Decay 10
    f"ts_decay_linear(trade_when(volume > adv20, 2 * {base_sales} + {base_soc} + {base_corp}, -1), 10)",
    
    # 2. Weight 2, Decay 20
    f"ts_decay_linear(trade_when(volume > adv20, 2 * {base_sales} + {base_soc} + {base_corp}, -1), 20)",
    
    # 3. Weight 3, Decay 10
    f"ts_decay_linear(trade_when(volume > adv20, 3 * {base_sales} + {base_soc} + {base_corp}, -1), 10)",
    
    # 4. Weight 3, Decay 20
    f"ts_decay_linear(trade_when(volume > adv20, 3 * {base_sales} + {base_soc} + {base_corp}, -1), 20)",
]
