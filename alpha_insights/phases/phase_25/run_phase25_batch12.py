...
def simulate(expression, trunc_val=0.08):
    data = {
        "type": "REGULAR",
        "settings": {
    ...
    while True:
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"Spawned: {sim_id} for T={trunc_val}, Expr={expression[:30]}")
                return sim_id
...
gr1 = "group_rank(ts_rank(ts_regression(sales, ts_step(1), 252, rettype=2), 60), sector)"
gr2 = "group_rank(ts_rank(scl12_sentiment_fast_d1, 20), sector)"
gr3 = "group_rank(ts_rank(-mean_corporate_action_sentiment, 20), sector)"

combo_bins5 = f"round({gr1} * 5) + round({gr2} * 5) + round({gr3} * 5)"
expr_1 = f"ts_decay_linear(trade_when(volume > adv20, {combo_bins5}, -1), 20)"

combo_bins2 = f"round({gr1} * 2) + round({gr2} * 2) + round({gr3} * 2)"
expr_2 = f"ts_decay_linear(trade_when(volume > adv20, {combo_bins2}, -1), 20)"

combo_normal = f"{gr1} + {gr2} + {gr3}"
expr_normal = f"ts_decay_linear(trade_when(volume > adv20, {combo_normal}, -1), 20)"
...
sims = []
for expr, trunc_val in runs:
    sims.append(simulate(expr, trunc_val))
    time.sleep(2)
