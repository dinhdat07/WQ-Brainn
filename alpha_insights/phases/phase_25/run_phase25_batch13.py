...
def simulate(expression, decay_val, trunc_val=0.08):
    data = {
        "type": "REGULAR",
        "settings": {
    ...
    while True:
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"Spawned: {sim_id} for Decay={decay_val}, Trunc={trunc_val}")
                return sim_id
...
sims = []
for decay_val, trunc_val in runs:
    expr = f"ts_decay_linear(trade_when(volume > adv20, {combo}, -1), {decay_val})"
    sims.append(simulate(expr, decay_val, trunc_val))
    time.sleep(2)
