...
combo = "group_rank(ts_rank(ts_regression(sales, ts_step(1), 252, rettype=2), 60), sector) + group_rank(ts_rank(scl12_sentiment_fast_d1, 20), sector) + group_rank(ts_rank(-mean_corporate_action_sentiment, 20), sector)"

exprs = [
    # 1. Exp Window decay 20
    f"ts_decay_exp_window(trade_when(volume > adv20, {combo}, -1), 20)",
    
    # 2. Bucket 10
    f"ts_decay_linear(trade_when(volume > adv20, bucket(10, {combo}), -1), 20)",
    
    # 3. Bucket 5
    f"ts_decay_linear(trade_when(volume > adv20, bucket(5, {combo}), -1), 20)",
    
    # 4. Strict Volume Filter
    f"ts_decay_linear(trade_when(volume > ts_mean(volume, 60), {combo}, -1), 20)",
]
