...
variants = {
    "Neutralize Sector + Liq": "trade_when(volume > ts_mean(volume, 20), ts_decay_linear(group_neutralize(ts_delta(anl4_afv4_eps_mean, 60), sector) - group_neutralize(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), sector), 5), -1)",
    "Neutralize Market + Liq": "trade_when(volume > ts_mean(volume, 20), ts_decay_linear(group_neutralize(ts_delta(anl4_afv4_eps_mean, 60), market) - group_neutralize(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), market), 5), -1)"
}
