...
exprs = [
    # 1. Competitor Graph
    "ts_decay_linear(group_zscore(pv13_ompetitorgraphrank_hub_rank, subindustry), 5)",
    
    # 2. Customer Graph
    "ts_decay_linear(group_zscore(pv13_ustomergraphrank_hub_rank, subindustry), 5)",
    
    # 3. Social Value (Sentiment Z-score)
    "ts_decay_linear(group_zscore(snt_social_value, subindustry), 5)",
    
    # 4. Social Volume
    "ts_decay_linear(group_zscore(snt_social_volume, subindustry), 5)",
    
    # 5. General Buzz (scl12_buzz)
    "ts_decay_linear(group_zscore(scl12_buzz, subindustry), 5)",
    
    # 6. Footnotes Debt Repayment / Cap
    "ts_decay_linear(group_zscore(fn_repayments_of_debt_a / cap, subindustry), 5)",
    
    # 7. Footnotes Debt Issuance / Cap
    "ts_decay_linear(group_zscore(fn_proceeds_from_issuance_of_debt_a / cap, subindustry), 5)"
]
