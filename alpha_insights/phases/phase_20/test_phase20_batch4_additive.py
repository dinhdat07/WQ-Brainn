...
exprs = [
    # 1. 3-factor sentiment + eps + reversion
    "ts_decay_linear(group_rank(-ts_delta(close, 5), subindustry) + group_rank(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), subindustry) + group_rank(ts_delta(anl4_afv4_eps_mean, 60), subindustry), 10)",
    # 2. 4-factor by adding industry momentum
    "ts_decay_linear(group_rank(-ts_delta(close, 5), subindustry) + group_rank(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), subindustry) + group_rank(ts_delta(anl4_afv4_eps_mean, 60), subindustry) + group_rank(-ts_mean(returns, 20), subindustry), 10)",
    # 3. Liquidity filter on 4-factor
    "trade_when(volume > ts_mean(volume, 20), ts_decay_linear(group_rank(-ts_delta(close, 5), subindustry) + group_rank(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), subindustry) + group_rank(ts_delta(anl4_afv4_eps_mean, 60), subindustry) + group_rank(-ts_mean(returns, 20), subindustry), 10), -1)",
    # 4. Multiplicative wrapping of additive ensemble
    "ts_decay_linear(rank(-ts_delta(close, 5)) * (group_rank(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), subindustry) + group_rank(ts_delta(anl4_afv4_eps_mean, 60), subindustry) + group_rank(-ts_mean(returns, 20), subindustry)), 10)",
]

sims = []
for i, expr in enumerate(exprs):
    print(f"\nStarting Batch 4 Idea {i+1}")
