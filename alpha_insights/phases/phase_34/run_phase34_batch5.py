...
expressions = [
    # 1. Reverse outer expression (Value vs Sentiment)
    "-1 * ts_decay_linear(group_rank(ts_corr(fscore_value, mean_composite_sentiment_score, 20), subindustry), 30)",
    
    # 2. Reverse outer expression (Quality vs Volatility)
    "-1 * ts_decay_linear(group_rank(ts_corr(fscore_quality, implied_volatility_call_30, 20), subindustry), 30)",
    
    # 3. Reverse outer expression (Risk vs Breakeven)
    "-1 * ts_decay_linear(group_rank(ts_corr(unsystematic_risk_last_30_days, option_breakeven_30, 20), subindustry), 30)",
    
    # 4. Standard expression from Risk vs Breakeven but cross-sectional difference
    "ts_decay_linear(group_rank(unsystematic_risk_last_30_days, subindustry) - group_rank(option_breakeven_30, subindustry), 30)"
]
