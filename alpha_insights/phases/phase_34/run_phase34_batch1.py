...
# Create Phase 34 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_34", exist_ok=True)
...
expressions = [
    # 1. Analyst vs Sentiment Divergence (Rank Delta)
    "ts_decay_linear(group_rank(rank(analyst_revision_rank_derivative) - rank(mean_composite_sentiment_score), subindustry), 30)",
    
    # 2. Quality vs Volatility Squeeze (Correlation)
    "ts_decay_linear(group_rank(ts_corr(fscore_quality, implied_volatility_call_30, 20), subindustry), 30)",
    
    # 3. Analyst Estimates vs Option Breakeven (Cross-Asset Arbitrage)
    "ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, option_breakeven_30, 20), subindustry), 30)",
    
    # 4. Value vs Sentiment (Fundamental vs Dumb Money)
    "ts_decay_linear(group_rank(ts_corr(fscore_value, mean_composite_sentiment_score, 20), subindustry), 30)"
]
