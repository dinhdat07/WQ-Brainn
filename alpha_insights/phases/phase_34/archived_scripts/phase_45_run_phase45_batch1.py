...
# Create Phase 45 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_45", exist_ok=True)
...
expressions = [
    # 1. Analyst vs Fundamentals (Market Neutral)
    "group_neutralize(ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, fscore_total, 180), subindustry), 10), market)",
    
    # 2. Analyst vs Fundamentals (Sector Neutral)
    "group_neutralize(ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, fscore_total, 180), subindustry), 10), sector)",
    
    # 3. Analyst vs Value (Market Neutral)
    "group_neutralize(ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, fscore_value, 180), subindustry), 10), market)",
    
    # 4. AI Earnings vs Fundamentals (Market Neutral)
    "group_neutralize(ts_decay_linear(group_rank(ts_corr(rp_nip_earnings, fscore_total, 180), subindustry), 10), market)"
]
