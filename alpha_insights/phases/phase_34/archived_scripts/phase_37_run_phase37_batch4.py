...
expressions = [
    # 1. 60-day smoothing with Sector
    "-1 * ts_decay_linear(group_rank(scl12_sentiment, sector) - group_rank(pcr_vol_all, sector), 60)",
    
    # 2. 90-day smoothing with Sector
    "-1 * ts_decay_linear(group_rank(scl12_sentiment, sector) - group_rank(pcr_vol_all, sector), 90)",
    
    # 3. 90-day smoothing with Subindustry
    "-1 * ts_decay_linear(group_rank(scl12_sentiment, subindustry) - group_rank(pcr_vol_all, subindustry), 90)",
    
    # 4. Fast Sentiment (scl12_sentiment_fast_d1) with 40-day smoothing
    "-1 * ts_decay_linear(group_rank(scl12_sentiment_fast_d1, subindustry) - group_rank(pcr_vol_all, subindustry), 40)"
]
