...
expressions = [
    ...
    "-1 * ts_decay_linear(group_rank(scl12_sentiment, subindustry) * group_rank(ts_decay_linear(shares_sold_short_count, 20), subindustry), 20)",
    
    # 2. Social Buzz vs Short Interest (FOMO Squeeze)
    "-1 * ts_decay_linear(group_rank(scl12_buzz, subindustry) * group_rank(ts_decay_linear(shares_sold_short_count, 20), subindustry), 20)",
    
    # 3. AI Earnings vs Short Interest (Earnings Squeeze)
    "-1 * ts_decay_linear(group_rank(rp_nip_earnings, subindustry) * group_rank(ts_decay_linear(shares_sold_short_count, 20), subindustry), 20)",
    
    # 4. Analyst Revisions vs Short Interest (Upgrade Squeeze)
    "-1 * ts_decay_linear(group_rank(analyst_revision_rank_derivative, subindustry) * group_rank(ts_decay_linear(shares_sold_short_count, 20), subindustry), 20)"
]
