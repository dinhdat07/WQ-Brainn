...
# Create Phase 44 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_44", exist_ok=True)
...
expressions = [
    # 1. Analyst vs Fundamentals (60-day Z-Score)
    "ts_decay_linear(group_rank(ts_zscore(analyst_revision_rank_derivative, 60) * ts_zscore(fscore_total, 60), subindustry), 60)",
    
    # 2. AI Earnings vs Fundamentals
    "ts_decay_linear(group_rank(ts_zscore(rp_nip_earnings, 60) * ts_zscore(fscore_total, 60), subindustry), 60)",
    
    # 3. Social Sentiment vs Fundamentals
    "ts_decay_linear(group_rank(ts_zscore(snt_social_value, 60) * ts_zscore(fscore_total, 60), subindustry), 60)",
    
    # 4. Option Breakeven vs Fundamentals
    "ts_decay_linear(group_rank(ts_zscore(option_breakeven_30, 60) * ts_zscore(fscore_total, 60), subindustry), 60)"
]
