...
# Create Phase 38 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_38", exist_ok=True)
...
expressions = [
    # 1. NIP Earnings vs PCR Vol
    "ts_decay_linear(group_rank(rp_nip_earnings, subindustry) - group_rank(pcr_vol_30, subindustry), 20)",
    
    # 2. NIP MNA vs PCR Vol
    "ts_decay_linear(group_rank(rp_nip_mna, subindustry) - group_rank(pcr_vol_30, subindustry), 20)",
    
    # 3. NIP Earnings vs Options Skew
    "ts_decay_linear(group_rank(rp_nip_earnings, subindustry) - group_rank(implied_volatility_mean_skew_30, subindustry), 20)",
    
    # 4. NIP Insider vs Options Skew
    "ts_decay_linear(group_rank(rp_nip_insider, subindustry) - group_rank(implied_volatility_mean_skew_30, subindustry), 20)"
]
