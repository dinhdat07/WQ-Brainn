...
expressions = [
    # 1. Extend Decay to 60 days to boost Margin (Divergence)
    "-1 * ts_decay_linear(group_rank(scl12_sentiment, subindustry) - group_rank(pcr_vol_all, subindustry), 60)",
    
    # 2. Extend Decay to 40 days, use Sector (Divergence)
    "-1 * ts_decay_linear(group_rank(scl12_sentiment, sector) - group_rank(pcr_vol_all, sector), 40)",
    
    # 3. Time-Series Correlation between Social Sentiment and PCR All
    "-1 * ts_decay_linear(group_rank(ts_corr(scl12_sentiment, pcr_vol_all, 20), subindustry), 30)",
    
    # 4. Time-Series Correlation between Social Buzz and PCR 30
    "-1 * ts_decay_linear(group_rank(ts_corr(scl12_buzz, pcr_vol_30, 20), subindustry), 30)"
]
