...
# Create Phase 51 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_51", exist_ok=True)
...
expressions = [
    # 1. Output Smoothing 90d on the 1.72 Sharpe model
    "ts_decay_linear(group_rank(ts_corr(scl12_sentiment, pcr_vol_60, 90), subindustry), 90)",
    
    # 2. Output Smoothing 120d on the 1.72 Sharpe model
    "ts_decay_linear(group_rank(ts_corr(scl12_sentiment, pcr_vol_60, 90), subindustry), 120)",
    
    # 3. Simple moving average 90d (ts_mean) instead of decay_linear
    "ts_mean(group_rank(ts_corr(scl12_sentiment, pcr_vol_60, 90), subindustry), 90)",
    
    # 4. Input Smoothing 20d + Output Smoothing 90d
    "ts_decay_linear(group_rank(ts_corr(ts_decay_linear(scl12_sentiment, 20), pcr_vol_60, 90), subindustry), 90)"
]
