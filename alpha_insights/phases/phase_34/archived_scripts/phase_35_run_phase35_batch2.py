...
expressions = [
    # 1. Standard Inverted Total F-Score
    "-1 * ts_decay_linear(group_rank(ts_corr(pcr_vol_30, fscore_total, 20), subindustry), 30)",
    
    # 2. Faster Window Total F-Score
    "-1 * ts_decay_linear(group_rank(ts_corr(pcr_vol_30, fscore_total, 10), subindustry), 20)",
    
    # 3. Growth with Longer Decay
    "-1 * ts_decay_linear(group_rank(ts_corr(pcr_vol_30, fscore_growth, 20), subindustry), 40)",
    
    # 4. Basic Fundamental Logic Score (BFL)
    "-1 * ts_decay_linear(group_rank(ts_corr(pcr_vol_30, fscore_bfl_total, 20), subindustry), 30)"
]
