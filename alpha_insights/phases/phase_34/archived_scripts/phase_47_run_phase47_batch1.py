...
# Create Phase 47 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_47", exist_ok=True)
...
expressions = [
    # 1. Spread Z-score (Fundamental Momentum vs Options)
    "ts_decay_linear(group_zscore(fscore_bfl_momentum, subindustry) - group_zscore(implied_volatility_call_30, subindustry), 60)",
    
    # 2. Z-Score Multiplication (AI Earnings vs Momentum)
    "ts_decay_linear(group_rank(ts_zscore(rp_nip_earnings, 20) * ts_zscore(fscore_bfl_momentum, 20), subindustry), 60)",
    
    # 3. AI Insider vs Options Flow (No fundamentals)
    "ts_decay_linear(group_rank(ts_corr(rp_nip_insider, pcr_vol_30, 60), subindustry), 60)",
    
    # 4. Analysts vs Options Breakeven (Z-score version)
    "ts_decay_linear(group_zscore(ts_corr(analyst_revision_rank_derivative, option_breakeven_60, 20), subindustry), 60)"
]
