...
# Create Phase 48 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_48", exist_ok=True)
...
expressions = [
    # 1. Quality instead of Value
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_insider, fscore_quality, 60), subindustry), 60)",
    
    # 2. Faster Smoothing (30 days instead of 60)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_insider, fscore_value, 60), subindustry), 30)",
    
    # 3. Faster Correlation (40 days instead of 60)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_insider, fscore_value, 40), subindustry), 60)",
    
    # 4. Spearman Rank Correlation
    "-1 * ts_decay_linear(group_rank(ts_corr(rank(rp_nip_insider), rank(fscore_value), 60), subindustry), 60)"
]
