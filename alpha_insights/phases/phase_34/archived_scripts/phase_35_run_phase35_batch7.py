...
expressions = [
    # 1. Global Rank (Not neutralized by subindustry initially)
    "-1 * ts_decay_linear(rank(analyst_revision_rank_derivative) - rank(pcr_vol_30), 40)",
    
    # 2. Sector Rank
    "-1 * ts_decay_linear(group_rank(analyst_revision_rank_derivative, sector) - group_rank(pcr_vol_30, sector), 40)",
    
    # 3. Faster Fear Window (10 days)
    "-1 * ts_decay_linear(group_rank(analyst_revision_rank_derivative, subindustry) - group_rank(pcr_vol_10, subindustry), 40)",
    
    # 4. Slower Fear Window (60 days)
    "-1 * ts_decay_linear(group_rank(analyst_revision_rank_derivative, subindustry) - group_rank(pcr_vol_60, subindustry), 40)"
]
