...
# Create Phase 42 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_42", exist_ok=True)
...
expressions = [
    # 1. AI Earnings vs Derivatives
    "ts_decay_linear(group_rank(ts_zscore(rp_nip_earnings, 20) * ts_zscore(option_breakeven_30, 20), subindustry), 60)",
    
    # 2. AI Insider vs Derivatives
    "ts_decay_linear(group_rank(ts_zscore(rp_nip_insider, 20) * ts_zscore(option_breakeven_30, 20), subindustry), 60)",
    
    # 3. Analyst Revisions vs Derivatives
    "ts_decay_linear(group_rank(ts_zscore(analyst_revision_rank_derivative, 20) * ts_zscore(option_breakeven_30, 20), subindustry), 60)",
    
    # 4. Double Derivatives (PCR Vol vs Breakeven)
    "ts_decay_linear(group_rank(ts_zscore(pcr_vol_30, 20) * ts_zscore(option_breakeven_30, 20), subindustry), 60)"
]
