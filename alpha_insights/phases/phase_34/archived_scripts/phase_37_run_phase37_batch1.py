...
# Create Phase 37 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_37", exist_ok=True)
...
expressions = [
    # 1. Retail Sentiment vs Wall Street (Global Rank)
    "ts_decay_linear(rank(scl12_sentiment) - rank(analyst_revision_rank_derivative), 10)",
    
    # 2. Retail Sentiment vs Smart Money Options (Sector Rank)
    "ts_decay_linear(group_rank(scl12_sentiment, sector) - group_rank(pcr_vol_10, sector), 10)",
    
    # 3. FOMO Buzz Correlation (Subindustry)
    "ts_decay_linear(group_rank(ts_corr(scl12_buzz, pcr_vol_10, 10), subindustry), 10)",
    
    # 4. Retail Sentiment vs Smart Money Options (Subindustry Rank)
    "ts_decay_linear(group_rank(scl12_sentiment, subindustry) - group_rank(pcr_vol_10, subindustry), 10)"
]
