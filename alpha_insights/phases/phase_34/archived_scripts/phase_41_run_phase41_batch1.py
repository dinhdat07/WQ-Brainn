...
# Create Phase 41 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_41", exist_ok=True)
...
expressions = [
    # 1. Analyst vs Value
    "ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, fscore_value, 60), subindustry), 60)",
    
    # 2. Analyst vs Growth
    "ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, fscore_growth, 60), subindustry), 60)",
    
    # 3. Analyst vs Momentum
    "ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, fscore_momentum, 60), subindustry), 60)",
    
    # 4. Analyst vs Sales Estimate
    "ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, sales_estimate_value, 60), subindustry), 60)"
]
