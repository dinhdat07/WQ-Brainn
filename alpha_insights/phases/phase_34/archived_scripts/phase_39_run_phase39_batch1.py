...
# Create Phase 39 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_39", exist_ok=True)
...
expressions = [
    # 1. Insider vs Value
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, fscore_value, 60), subindustry), 60)",
    
    # 2. Insider vs Growth
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, fscore_growth, 60), subindustry), 60)",
    
    # 3. Insider vs Momentum
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, fscore_momentum, 60), subindustry), 60)",
    
    # 4. Insider vs Analyst Revisions
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, analyst_revision_rank_derivative, 60), subindustry), 60)"
]
