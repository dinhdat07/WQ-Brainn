...
# Create Phase 43 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_43", exist_ok=True)
...
expressions = [
    # 1. Social FOMO vs News Short Interest
    "ts_decay_linear(group_rank(ts_zscore(scl12_sentiment, 20) * ts_zscore(news_short_interest, 20), subindustry), 60)",
    
    # 2. Social FOMO vs Professional Short Interest (NWS12)
    "ts_decay_linear(group_rank(ts_zscore(scl12_sentiment, 20) * ts_zscore(nws12_mainz_short_interest, 20), subindustry), 60)",
    
    # 3. AI Insider vs News Short Interest
    "ts_decay_linear(group_rank(ts_zscore(rp_nip_insider, 20) * ts_zscore(news_short_interest, 20), subindustry), 60)",
    
    # 4. Options Volume vs News Short Interest
    "ts_decay_linear(group_rank(ts_zscore(pcr_vol_30, 20) * ts_zscore(news_short_interest, 20), subindustry), 60)"
]
