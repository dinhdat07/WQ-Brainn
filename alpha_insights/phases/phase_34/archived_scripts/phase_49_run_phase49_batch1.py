...
# Create Phase 49 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_49", exist_ok=True)
...
expressions = [
    # 1. Sales Estimates vs Option Breakeven
    "-1 * ts_decay_linear(group_rank(ts_corr(sales_estimate_value, option_breakeven_30, 20), subindustry), 60)",
    
    # 2. Put-Call Volume vs News Short Interest
    "-1 * ts_decay_linear(group_rank(ts_corr(pcr_vol_30, news_short_interest, 20), subindustry), 60)",
    
    # 3. Operating Margin vs Short Interest
    "-1 * ts_decay_linear(group_rank(ts_corr(operating_margin, short_interest, 60), subindustry), 60)",
    
    # 4. AI Revenue News vs Put-Call Open Interest
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_revenue, pcr_oi_30, 40), subindustry), 60)"
]
