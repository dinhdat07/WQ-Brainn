...
# Create Phase 40 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_40", exist_ok=True)
...
expressions = [
    # 1. Social Sentiment vs Short Interest (Meme Stock Squeeze)
    "ts_decay_linear(group_rank(scl12_sentiment, subindustry) * group_rank(shares_sold_short_count, subindustry), 20)",
    
    # 2. Social Buzz vs Short Interest (FOMO Squeeze)
    "ts_decay_linear(group_rank(scl12_buzz, subindustry) * group_rank(shares_sold_short_count, subindustry), 20)",
    
    # 3. AI Earnings vs Short Interest (Earnings Squeeze)
    "ts_decay_linear(group_rank(rp_nip_earnings, subindustry) * group_rank(shares_sold_short_count, subindustry), 20)",
    
    # 4. Analyst Revisions vs Short Interest (Upgrade Squeeze)
    "ts_decay_linear(group_rank(analyst_revision_rank_derivative, subindustry) * group_rank(shares_sold_short_count, subindustry), 20)"
]
