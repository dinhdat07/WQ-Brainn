...
# Create Phase 35 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_35", exist_ok=True)
...
expressions = [
    # 1. Options Volatility Skew (Put vs Call)
    "ts_decay_linear(group_rank(implied_volatility_put_30 - implied_volatility_call_30, subindustry), 30)",
    
    # 2. PCR vs Total F-Score Divergence
    "ts_decay_linear(group_rank(ts_corr(pcr_vol_30, fscore_total, 20), subindustry), 30)",
    
    # 3. PCR vs Growth Divergence
    "ts_decay_linear(group_rank(ts_corr(fscore_growth, pcr_oi_all, 20), subindustry), 30)",
    
    # 4. Systematic Risk vs Earnings Certainty
    "ts_decay_linear(group_rank(ts_corr(unsystematic_risk_last_30_days, earnings_certainty_rank_derivative, 20), subindustry), 30)"
]
