...
# Create Phase 46 directory if not exists
os.makedirs("e:/CODING/MMO/wq-brain/WQ-Brainn/alpha_insights/phases/phase_46", exist_ok=True)
...
expressions = [
    # 1. Inverted AI Insider vs Value (60 days) -> targeting 1.37 Sharpe
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_insider, fscore_value, 60), subindustry), 60)",
    
    # 2. Inverted AI Insider vs Total FScore (60 days)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_insider, fscore_total, 60), subindustry), 60)",
    
    # 3. Inverted AI Earnings vs Value (60 days)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_earnings, fscore_value, 60), subindustry), 60)",
    
    # 4. Inverted AI Earnings vs Total FScore (60 days)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_nip_earnings, fscore_total, 60), subindustry), 60)"
]
                ...
                while True:
                    ...
                    if alpha_resp.status_code == 200:
                        ...
                        if is_metrics:
                            ...
                            print(f'Subuniverse Sharpe (approx via turnover/margin): check WQ Brain API response directly if needed.')
                            break
                        else:
                            time.sleep(5)
            break
