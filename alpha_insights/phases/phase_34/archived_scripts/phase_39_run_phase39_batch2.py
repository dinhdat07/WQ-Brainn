...
def simulate(expression, trunc_val=0.08, neut="SECTOR"):
    data = {
        "type": "REGULAR",
        "settings": {
...
expressions = [
    # 1. Insider vs Value (Sector)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, fscore_value, 60), sector), 60)",
    
    # 2. Insider vs Growth (Sector)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, fscore_growth, 60), sector), 60)",
    
    # 3. Insider vs Momentum (Sector)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, fscore_momentum, 60), sector), 60)",
    
    # 4. Insider vs Analyst Revisions (Sector)
    "-1 * ts_decay_linear(group_rank(ts_corr(rp_css_insider, analyst_revision_rank_derivative, 60), sector), 60)"
]
