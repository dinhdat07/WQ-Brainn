...
expressions = [
    # 1. Faster Window (10, 10)
    "-1 * ts_decay_linear(group_rank(ts_corr(fscore_value, mean_composite_sentiment_score, 10), subindustry), 10)",
    
    # 2. Slower Window (60, 60)
    "-1 * ts_decay_linear(group_rank(ts_corr(fscore_value, mean_composite_sentiment_score, 60), subindustry), 60)",
    
    # 3. Total F-Score vs Sentiment
    "-1 * ts_decay_linear(group_rank(ts_corr(fscore_total, mean_composite_sentiment_score, 20), subindustry), 30)",
    
    # 4. Smart Money vs Dumb Money (Analyst vs Sentiment)
    "-1 * ts_decay_linear(group_rank(ts_corr(analyst_revision_rank_derivative, mean_composite_sentiment_score, 20), subindustry), 30)"
]
