...
exprs = [
    # 1. Market neutralization with group_rank
    "ts_decay_linear(group_rank(implied_volatility_call_30 - implied_volatility_put_30, market), 20)",
    
    # 2. Market neutralization with group_zscore and longer decay
    "ts_decay_linear(group_zscore(implied_volatility_call_30 - implied_volatility_put_30, market), 20)",
    
    # 3. Percentage Skew
    "ts_decay_linear(group_rank((implied_volatility_call_30 - implied_volatility_put_30) / implied_volatility_call_30, subindustry), 15)",
    
    # 4. Multi-factor: IV Skew + Short Interest proxy (PCR)
    "ts_decay_linear(group_rank(implied_volatility_call_30 - implied_volatility_put_30, subindustry) + group_rank(-pcr_oi_30, subindustry), 20)",
    
    # 5. Volatility anomaly combined: Call Skew + Low Volatility
    "ts_decay_linear(group_rank(implied_volatility_call_30 - implied_volatility_put_30, subindustry) + group_rank(-historical_volatility_30, subindustry), 20)"
]
