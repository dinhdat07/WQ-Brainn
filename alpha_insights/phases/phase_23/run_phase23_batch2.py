...
exprs = [
    # 1. Analyst Sales Dispersion (avoid abs on events, use trade_when to avoid div by zero)
    "ts_decay_linear(group_rank(trade_when(anl4_fs_detail_estimates_basic_af_v4_nd_sales_median > 0, -(anl4_fs_detail_estimates_basic_af_v4_nd_sales_high - anl4_fs_detail_estimates_basic_af_v4_nd_sales_low) / anl4_fs_detail_estimates_basic_af_v4_nd_sales_median, 0), subindustry), 15)",
    
    # 2. Footnotes: Debt Issuance (without cap division to see if unit error)
    "ts_decay_linear(group_rank(-fn_proceeds_from_issuance_of_debt_a, subindustry), 15)",
    
    # 3. Footnotes: Debt Issuance / Sales (matching units)
    "ts_decay_linear(group_rank(-fn_proceeds_from_issuance_of_debt_a / sales, subindustry), 15)"
]
