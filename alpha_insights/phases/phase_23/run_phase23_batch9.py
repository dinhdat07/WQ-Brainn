...
exprs = [
    # 1. Decay 10
    "ts_decay_linear(group_rank(-analyst_revision_rank_derivative, subindustry) + group_rank(-ts_delta(close, 5), subindustry) + group_rank(implied_volatility_call_60 - historical_volatility_60, subindustry), 10)",
    
    # 2. Fast Reversion (delta 3) and Decay 12
    "ts_decay_linear(group_rank(-analyst_revision_rank_derivative, subindustry) + group_rank(-ts_delta(close, 3), subindustry) + group_rank(implied_volatility_call_60 - historical_volatility_60, subindustry), 12)",
    
    # 3. VWAP Divergence
    "ts_decay_linear(group_rank(-analyst_revision_rank_derivative, subindustry) + group_rank(vwap - close, subindustry) + group_rank(implied_volatility_call_60 - historical_volatility_60, subindustry), 12)",
    
    # 4. Long Term VRP
    "ts_decay_linear(group_rank(-analyst_revision_rank_derivative, subindustry) + group_rank(-ts_delta(close, 3), subindustry) + group_rank(implied_volatility_call_270 - historical_volatility_180, subindustry), 15)"
]
