...
exprs = [
    # 1. Analyst Revision Rank Derivative
    "ts_decay_linear(group_zscore(analyst_revision_rank_derivative, subindustry), 10)",
    
    # 2. Growth Potential Rank Derivative
    "ts_decay_linear(group_zscore(growth_potential_rank_derivative, subindustry), 10)",
    
    # 3. Volatility Risk Premium
    "ts_decay_linear(group_zscore(-(implied_volatility_call_60 - historical_volatility_60), subindustry), 10)",
    
    # 4. Analyst Recommendation Delta (1 is Strong Buy, 5 is Strong Sell) -> Delta > 0 means downgrade -> short
    "ts_decay_linear(group_zscore(-ts_delta(anl4_fs_detail_rec_v4_nd_estimate, 30), subindustry), 10)",
    
    # 5. Volatility Spread 2
    "ts_decay_linear(group_zscore(-(historical_volatility_30 - parkinson_volatility_30), subindustry), 10)"
]
