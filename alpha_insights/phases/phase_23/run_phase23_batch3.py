...
exprs = [
    # 1. Accruals Anomaly (Short High Accruals)
    "ts_decay_linear(group_zscore(-(net_income - cashflow_op) / assets, market), 15)",
    
    # 2. Betting Against Beta (Short High Beta)
    "ts_decay_linear(group_zscore(-beta_last_60_days_spy, market), 15)",
    
    # 3. PCR all change
    "ts_decay_linear(group_zscore(ts_delta(pcr_oi_all, 10), market), 15)",
    
    # 4. News Sentiment 
    "ts_decay_linear(group_zscore(ts_delta(nws18_ssc, 5), market), 15)",
    
    # 5. Social Volume Spike
    "ts_decay_linear(group_zscore(ts_delta(scl12_buzz, 3), market), 15)"
]
