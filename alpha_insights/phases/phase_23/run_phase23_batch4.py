...
exprs = [
    # 1. Social Sentiment Core
    "ts_decay_linear(group_zscore(scl12_sentiment, subindustry), 15)",
    
    # 2. Earnings Evaluation Sentiment (News)
    "ts_decay_linear(group_zscore(mean_earnings_evaluation_sentiment, subindustry), 15)",
    
    # 3. Optimal Position (US News Indicator)
    "ts_decay_linear(group_zscore(optimal_position_indicator, market), 15)",
    
    # 4. Cash to Short-term Debt (Fundamental Liquidity)
    "ts_decay_linear(group_zscore(trade_when(fnd6_np > 0, fnd6_ch / fnd6_np, 0), subindustry), 15)"
]
