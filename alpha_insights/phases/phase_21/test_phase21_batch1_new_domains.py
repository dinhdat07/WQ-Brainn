...
exprs = [
    # 1. Pure New Data Quad-Factor
    "ts_decay_linear(group_rank(fscore_bfl_value + fscore_bfl_profitability, subindustry) + group_rank(-unsystematic_risk_last_30_days, subindustry) + group_rank(ts_sum(snt_social_value, 5), subindustry) + group_rank(sales_estimate_value / cap, subindustry), 15)",
    
    # 2. New Data + Traditional Core (Safe Base)
    "ts_decay_linear(group_rank(sales / assets, subindustry) + group_rank(-unsystematic_risk_last_30_days, subindustry) + group_rank(ts_sum(snt_social_value, 5), subindustry) + group_rank(-(close - open) / open, subindustry), 15)",
    
    # 3. Alternative News Quad-Factor
    "ts_decay_linear(group_rank(fscore_bfl_momentum, subindustry) + group_rank(-systematic_risk_last_90_days, subindustry) + group_rank(optimal_position_indicator, subindustry) + group_rank(sales_estimate_value / cap, subindustry), 15)",
    
    # 4. Fundamental Scores Triad
    "ts_decay_linear(group_rank(fscore_bfl_value, subindustry) + group_rank(fscore_bfl_quality, subindustry) + group_rank(fscore_bfl_profitability, subindustry), 10)"
]
