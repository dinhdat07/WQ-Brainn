...
core_signal = "group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(ts_backfill(fn_repayments_of_debt_a, 252) / cap, sector) + group_rank(-analyst_revision_rank_derivative, sector)"

expressions = [
    f"ts_decay_linear({core_signal}, 10)",
    f"ts_decay_linear({core_signal}, 15)",
    f"ts_decay_linear(trade_when(volume > adv20, {core_signal}, -1), 10)",
    f"ts_decay_linear(trade_when(volume > adv20, {core_signal}, -1), 15)",
    f"ts_decay_linear(trade_when(volume > adv20, {core_signal}, -1), 20)"
]
