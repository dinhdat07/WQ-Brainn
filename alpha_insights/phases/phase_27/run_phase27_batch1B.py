...
expressions = [
    # 1. Competitor Authority Score
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_auth_rank, sector), 10)",
    
    # 2. Customer Authority Score
    "ts_decay_linear(group_rank(pv13_ustomergraphrank_auth_rank, sector), 10)",
    
    # 3. Social Sentiment Conviction (fast_d1)
    "ts_decay_linear(group_rank(snt_social_value_fast_d1 * snt_social_volume_fast_d1, sector), 10)",
    
    # 4. Short Term Investment to Assets ratio (Safety / Cash rich)
    "ts_decay_linear(group_rank(cash_st / assets, sector), 10)"
]
