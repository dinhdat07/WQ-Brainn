...
# We want to test raw data field availability and raw structural strength
expressions = [
    # 1. Test Competitor Network Centrality (with 'ompetitor' as found in doc)
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector), 10)",
    
    # 2. Test Competitor Network Centrality (with 'competitor' just in case of typo)
    "ts_decay_linear(group_rank(pv13_competitorgraphrank_hub_rank, sector), 10)",
    
    # 3. Test Idiosyncratic Risk (we want negative, low risk = good)
    "ts_decay_linear(group_rank(-unsystematic_risk_last_60_days, sector), 10)",
    
    # 4. Test Earnings Certainty Rank Derivative
    "ts_decay_linear(group_rank(earnings_certainty_rank_derivative, sector), 10)",
    
    # 5. Combined Triad test
    "ts_decay_linear(group_rank(pv13_competitorgraphrank_hub_rank, sector) + group_rank(-unsystematic_risk_last_60_days, sector) + group_rank(earnings_certainty_rank_derivative, sector), 20)"
]
