...
expressions = [
    # 1. Faster decay on the raw Hub Rank to boost turnover and fitness
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector), 4)",
    
    # 2. Delta of Hub Rank (momentum of network position)
    "ts_decay_linear(group_rank(ts_delta(pv13_ompetitorgraphrank_hub_rank, 60), sector), 10)",
    
    # 3. Additive Ensemble: Hub Rank + Earnings Certainty
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-earnings_certainty_rank_derivative, sector), 10)",
    
    # 4. Additive Ensemble: Hub Rank + Idiosyncratic Risk (Negative is better)
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-unsystematic_risk_last_60_days, sector), 10)",
    
    # 5. Triad: Hub Rank + Earnings Certainty + Idiosyncratic Risk
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-earnings_certainty_rank_derivative, sector) + group_rank(-unsystematic_risk_last_60_days, sector), 10)",
    
    # 6. Test Customer Graph Hub Rank baseline
