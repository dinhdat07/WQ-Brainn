...
expressions = [
    # 1. Duo with Decay 30
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector), 30)",
    
    # 2. Triad with Options
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(pcr_oi_all, sector), 20)",
    
    # 3. Triad with Short Beta
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(-beta_last_60_days_spy, sector), 20)",
    
    # 4. Triad with Short Volatility
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(-historical_volatility_60, sector), 20)"
]
