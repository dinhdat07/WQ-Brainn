...
expressions = [
    # 1. Earning + Value + Inventory
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(-earnings_certainty_rank_derivative, sector) + group_rank(-relative_valuation_rank_derivative, sector), 15)",
    
    # 2. Earning + Value + Options
    "ts_decay_linear(group_rank(-earnings_certainty_rank_derivative, sector) + group_rank(-relative_valuation_rank_derivative, sector) + group_rank(pcr_oi_all, sector), 15)",
    
    # 3. Inventory + Earning + Options
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(-earnings_certainty_rank_derivative, sector) + group_rank(pcr_oi_all, sector), 15)",
    
    # 4. Pure Derivatives Pair
    "ts_decay_linear(group_rank(-earnings_certainty_rank_derivative, sector) + group_rank(-relative_valuation_rank_derivative, sector), 15)"
]
