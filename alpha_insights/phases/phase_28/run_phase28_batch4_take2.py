...
expressions = [
    # 1. Machinery Value Signal (Value style)
    "ts_decay_linear(group_rank(ts_backfill(gross_machinery_equipment_value_2, 252) / cap, sector), 10)",
    
    # 2. Debt Expansion Signal (Short high debt issuers)
    "ts_decay_linear(group_rank(-ts_backfill(fn_proceeds_from_issuance_of_debt_a, 252) / cap, sector), 10)",
    
    # 3. Cash Yield (Defensive style)
    "ts_decay_linear(group_rank(ts_backfill(cash_st, 252) / cap, sector), 10)",
    
    # 4. Domestic Revenue Ratio
    "ts_decay_linear(group_rank(ts_backfill(domestic_revenue_amount, 252) / ts_backfill(sales, 252), sector), 10)"
]
