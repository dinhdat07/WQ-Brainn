...
expressions = [
    # 1. Analyst Disagreement (Short high disagreement)
    "ts_decay_linear(group_rank(-(sales_estimate_maximum - sales_estimate_minimum) / sales_estimate_median, sector), 10)",
    
    # 2. Options Put-Call Sentiment
    "ts_decay_linear(group_rank(pcr_oi_all, sector), 10)",
    
    # 3. Idiosyncratic Risk (Short high risk)
    "ts_decay_linear(group_rank(-unsystematic_risk_last_60_days, sector), 10)",
    
    # 4. Inventory Pile-up (Short high inventory-to-sales)
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector), 10)"
]
