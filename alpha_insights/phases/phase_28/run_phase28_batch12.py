...
expressions = [
    # 1. Delta of Inventory / Sales (60 days)
    "ts_decay_linear(group_rank(-ts_delta(ts_backfill(inventory, 252) / ts_backfill(sales, 252), 60), sector), 20)",
    
    # 2. Time-series Rank of Inventory / Sales (252 days)
    "ts_decay_linear(group_rank(-ts_rank(ts_backfill(inventory, 252) / ts_backfill(sales, 252), 252), sector), 20)",
    
    # 3. Base Inventory/Sales
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector), 20)",
    
    # 4. Base Asset Turnover
    "ts_decay_linear(group_rank(-ts_backfill(assets, 252) / ts_backfill(sales, 252), sector), 20)"
]
