...
expressions = [
    # 1. Subindustry internal neutralization
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, subindustry) + group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), subindustry), 20)",
    
    # 2. Weight 60% Inventory / 40% Hub
    "ts_decay_linear(0.4 * group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + 0.6 * group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector), 20)",
    
    # 3. Weight 40% Inventory / 60% Hub
    "ts_decay_linear(0.6 * group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + 0.4 * group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector), 20)",
    
    # 4. Decay 15 (Shorter decay to boost Sharpe?)
    "ts_decay_linear(group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector), 15)"
]
