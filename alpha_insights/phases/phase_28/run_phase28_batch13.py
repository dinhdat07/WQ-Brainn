...
expressions = [
    # 1. New Sentiment Score
    "ts_decay_linear(group_rank(mean_equity_sentiment_score, sector), 10)",
    
    # 2. Beta
    "ts_decay_linear(group_rank(-beta_last_60_days_spy, subindustry), 10)",
    
    # 3. Inv + Cust Hub (Subindustry)
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), subindustry) + group_rank(pv13_ustomergraphrank_hub_rank, subindustry), 10)",
    
    # 4. Inv + Cust Hub (Sector)
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(pv13_ustomergraphrank_hub_rank, sector), 10)"
]
