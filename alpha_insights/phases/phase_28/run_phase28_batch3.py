...
expressions = [
    # 1. Anomaly Triad (Inventory + Hub + Sentiment)
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(ts_rank(scl12_sentiment_fast_d1, 20), sector), 20)",
    
    # 2. The Ultimate Pentagon (Inventory + Hub + Sentiment + Sales Trend + Debt)
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(ts_rank(scl12_sentiment_fast_d1, 20), sector) + group_rank(ts_rank(ts_regression(sales, ts_step(1), 252, rettype=2), 60), sector) + group_rank(ts_backfill(fn_repayments_of_debt_a, 252) / cap, sector), 20)",
    
    # 3. Anomaly Triad 2 (Inventory + Hub + Put-Call Ratio)
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(pcr_oi_all, sector), 20)",
    
    # 4. Same as 3 but with decay 10 to increase turnover if needed
    "ts_decay_linear(group_rank(-ts_backfill(inventory, 252) / ts_backfill(sales, 252), sector) + group_rank(pv13_ompetitorgraphrank_hub_rank, sector) + group_rank(pcr_oi_all, sector), 10)"
]
