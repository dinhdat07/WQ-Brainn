...
expressions = [
    # 1. Receivables Anomaly (High Receivables to Sales is bad)
    "ts_decay_linear(group_rank(-ts_backfill(accounts_receivable, 252) / ts_backfill(sales, 252), sector), 10)",
    
    # 2. Gross Margin (High is good)
    "ts_decay_linear(group_rank(ts_backfill(sales - cogs, 252) / ts_backfill(sales, 252), sector), 10)",
    
    # 3. Operating Margin (High is good)
    "ts_decay_linear(group_rank(ts_backfill(operating_income, 252) / ts_backfill(sales, 252), sector), 10)",
    
    # 4. Asset Growth / Capex Anomaly (High Capex/Assets is bad)
    "ts_decay_linear(group_rank(-ts_backfill(capital_expenditure, 252) / ts_backfill(total_assets, 252), sector), 10)"
]
