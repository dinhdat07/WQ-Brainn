...
expressions = [
    # 1. Analyst Revision Score
    "ts_decay_linear(group_rank(analyst_revision_rank_derivative, sector), 10)",
    
    # 2. Composite Factor Score
    "ts_decay_linear(group_rank(composite_factor_score_derivative, sector), 10)",
    
    # 3. Asset Turnover (Short low turnover)
    "ts_decay_linear(group_rank(-ts_backfill(assets, 252) / ts_backfill(sales, 252), sector), 10)",
    
    # 4. Earning Yield (Long high earnings yield)
    "ts_decay_linear(group_rank(ts_backfill(net_income, 252) / cap, sector), 10)"
]
