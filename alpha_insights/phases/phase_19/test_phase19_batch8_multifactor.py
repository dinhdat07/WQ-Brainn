...
formulas = {
    "1. Multi-Factor (ML + Debt + Reversion)": "trade_when(volume > adv20, ts_decay_linear(rank(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry)) + rank(group_zscore(fn_repayments_of_debt_a / cap, sector)) + rank(group_neutralize(-(close - ts_mean(close, 5))/ts_mean(close, 5), subindustry)), 10), -1)",
    "2. Multi-Factor (ML + Reversion)": "trade_when(volume > adv20, ts_decay_linear(rank(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry)) + rank(group_neutralize(-(close - ts_mean(close, 5))/ts_mean(close, 5), subindustry)), 10), -1)",
    "3. Multi-Factor Z-Scored": "trade_when(volume > adv20, ts_decay_linear(group_zscore(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), sector) + group_zscore(-(close - ts_mean(close, 5))/ts_mean(close, 5), sector), 10), -1)",
}
