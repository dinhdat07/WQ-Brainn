...
formulas = {
    "1. ML Revision * Slow Reversion": "trade_when(volume > adv20, ts_decay_linear(group_zscore(analyst_revision_rank_derivative * -(close - ts_mean(close, 10)), market), 10), -1)",
    "2. ML Revision * Social Sentiment": "trade_when(volume > adv20, ts_decay_linear(group_zscore(analyst_revision_rank_derivative * (ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60)), market), 10), -1)",
    "3. ML Revision + Social Sentiment": "trade_when(volume > adv20, ts_decay_linear(group_zscore(analyst_revision_rank_derivative, market) - group_zscore(ts_sum(mean_composite_sentiment_score, 10) - ts_sum(mean_composite_sentiment_score, 60), market), 10), -1)",
    "4. Fast Reversion (Market Norm)": "trade_when(volume > adv20, ts_decay_linear(group_zscore(analyst_revision_rank_derivative * -(close-open)/open, market), 10), -1)"
}
