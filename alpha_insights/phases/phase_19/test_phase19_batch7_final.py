...
formulas = {
    "1. ML Revision x Phase18 Reversion": "trade_when(volume > adv20, ts_decay_linear(rank(group_neutralize(analyst_revision_rank_derivative * -(close - ts_mean(close, 5))/ts_mean(close, 5), subindustry)), 10), -1)",
    "2. ML Revision (Pure, adv20)": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 10), -1)",
    "3. Debt Repayment + ML Revision": "trade_when(volume > adv20, ts_decay_linear(rank(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry)) + rank(group_zscore(fn_repayments_of_debt_a / cap, sector)), 10), -1)",
}
