...
formulas = {
    "1. Rank Inside Decay": "trade_when(volume > adv20, ts_decay_linear(rank(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry)) - 0.5, 10), -1)",
    "2. Z-Score Inside Decay": "trade_when(volume > adv20, ts_decay_linear(group_zscore(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 10), -1)",
    "3. Debt Repayment * Reversion (Z-Score)": "trade_when(volume > adv20, ts_decay_linear(group_zscore(fn_repayments_of_debt_a / cap, subindustry) * -(close-open)/open, 10), -1)"
}
