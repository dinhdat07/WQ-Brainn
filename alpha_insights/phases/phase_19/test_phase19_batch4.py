...
formulas = {
    "1. IV Premium Reversion": "trade_when(volume > adv20, ts_decay_linear(group_zscore(implied_volatility_call_10 - historical_volatility_10, sector) * group_zscore(-(close-open)/open, sector), 10), -1)",
    "2. Option Breakeven Spread": "trade_when(volume > adv20, ts_decay_linear(group_zscore((option_breakeven_30 - close)/close, subindustry), 10), -1)",
    "3. Composite Momentum ML": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(composite_factor_score_derivative, subindustry), 5), -1)",
    "4. Supply Chain Momentum": "trade_when(volume > adv20, ts_decay_linear(group_zscore(pv13_ustomergraphrank_auth_rank, sector) * group_zscore(returns, sector), 10), -1)",
    "5. Sales Estimate vs Price": "trade_when(volume > adv20, ts_decay_linear(group_zscore(sales_estimate_value, sector) - group_zscore(close, sector), 10), -1)"
}
