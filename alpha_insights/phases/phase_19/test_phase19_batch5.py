...
formulas = {
    "1. Decay 20": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 20), -1)",
    "2. Decay 30": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 30), -1)",
    "3. Smooth Signal": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(ts_mean(analyst_revision_rank_derivative, 5) * -(close-open)/open, subindustry), 10), -1)",
    "4. Cross-Sectional Rank": "trade_when(volume > adv20, rank(ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 10)), -1)",
    "5. Volume Filter Upgrade": "trade_when(volume > adv60, ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 10), -1)"
}
