...
def check_correlation(alpha_id):
    # Check against known phase 10/17/18 alphas
...
formulas = {
    "1. ML Analyst Momentum": "trade_when(volume > adv20, ts_decay_linear(group_zscore(analyst_revision_rank_derivative, subindustry) + group_zscore(-(close-open)/open, subindustry), 10), -1)",
    "2. ML Revision * Reversion": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 10), -1)",
    "3. Social Media Sentiment Volume": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(snt_social_value * snt_social_volume, sector), 5), -1)",
    "4. Ravenpack High Conf Sentiment": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(nws18_qcm * nws18_ssc, sector), 10), -1)",
    "5. Unsystematic Risk Reversion": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(-unsystematic_risk_last_30_days, sector) * group_neutralize(-(close-open)/open, sector), 10), -1)"
}
