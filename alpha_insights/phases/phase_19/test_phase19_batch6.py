...
formulas = {
    "1. ML Revision x Phase18 Reversion": "trade_when(volume > adv60, ts_decay_linear(rank(group_neutralize(analyst_revision_rank_derivative * -(close - ts_mean(close, 5)), subindustry)), 10), -1)",
    "2. Footnotes Debt Issuance Penalty": "trade_when(volume > adv20, ts_decay_linear(rank(-group_zscore(fn_proceeds_from_issuance_of_debt_a / cap, sector)), 10), -1)",
    "3. Footnotes Debt Repayment Alpha": "trade_when(volume > adv20, ts_decay_linear(rank(group_zscore(fn_repayments_of_debt_a / cap, sector)), 10), -1)",
    "4. ML Earnings Certainty x Reversion": "trade_when(volume > adv60, ts_decay_linear(group_neutralize(earnings_certainty_rank_derivative * -(close-open)/open, subindustry), 10), -1)",
    "5. ML Growth Potential x Reversion": "trade_when(volume > adv60, ts_decay_linear(group_neutralize(growth_potential_rank_derivative * -(close-open)/open, subindustry), 10), -1)"
}
