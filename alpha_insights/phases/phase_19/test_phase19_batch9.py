...
formulas = {
    "1. Earnings Certainty x Reversion": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(earnings_certainty_rank_derivative * -(close-open)/open, subindustry), 10), -1)",
    "2. Growth Potential x Reversion": "trade_when(volume > adv20, ts_decay_linear(group_neutralize(growth_potential_rank_derivative * -(close-open)/open, subindustry), 10), -1)",
    "3. Debt Repayment x Reversion": "trade_when(volume > adv20, ts_decay_linear(group_neutralize((fn_repayments_of_debt_a / cap) * -(close-open)/open, subindustry), 10), -1)",
    "4. ML Revision + Volume Filter Trend": "trade_when(adv20 > ts_mean(volume, 120), ts_decay_linear(group_neutralize(analyst_revision_rank_derivative * -(close-open)/open, subindustry), 10), -1)"
}
