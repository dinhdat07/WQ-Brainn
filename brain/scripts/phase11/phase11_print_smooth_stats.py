...
def main():
    ...
    with open('E:/CODING/MMO/wq-brain/WQ-Brainn/brain/scripts/phase11/phase11_smooth_pending.json', 'r') as f:
        data = json.load(f)

    print(f"{'Name':<30} | {'Status':<10} | {'Alpha ID':<8} | {'Sharpe':<6} | {'Fitness':<7} | {'Returns':<7} | {'Turnover':<8} | {'Margin':<8}")
    print('-'*105)
    ...
    for row in data:
        ...
        if status == 'COMPLETE' and isinstance(alpha_id, str):
            ...
            if not is_stats:
                print(f"{name:<30} | {status:<10} | {alpha_id:<8} | {'N/A':<6} | {'N/A':<7} | {'N/A':<7} | {'N/A':<8} | {'N/A':<8}")
                continue
            ...
            print(f"{name:<30} | {status:<10} | {alpha_id:<8} | {sharpe:6.2f} | {fitness:7.2f} | {returns:7.2%} | {turnover:8.2%} | {margin:8.4f}")
        else:
            print(f"{name:<30} | {status:<10} | {'N/A':<8} | {'N/A':<6} | {'N/A':<7} | {'N/A':<7} | {'N/A':<8} | {'N/A':<8}")
