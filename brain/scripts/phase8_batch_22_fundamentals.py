...
def main():
    file_logger.info("Starting Phase 8 Batch 22 - 10: Fundamental Z-Score x Fast Reversion...")
    ...
    # 5-day Reversion
    rev_5d = "rank(-(close / ts_mean(close, 5)))"
    # Intraday Reversion
    rev_intra = "rank(-(close - open) / open)"
    # 1-day Return Reversion
    rev_1d = "rank(-returns)"
    # 3-day Reversion
    rev_3d = "rank(-ts_delta(close, 3))"

    tasks = [
        ("22_8_AssetTurn_RevIntra", f"ts_decay_linear(group_zscore((sales / assets) * {rev_intra}, subindustry), 5)", {}),
        ("22_9_FCFPrice_Rev3d", f"ts_decay_linear(group_zscore(((income + depreciation - capex) / (sharesout * close)) * {rev_3d}, subindustry), 5)", {}),
        ("22_10_GrossMargin_RevIntra", f"ts_decay_linear(group_zscore(((sales - cogs) / sales) * {rev_intra}, subindustry), 5)", {})
    ]
