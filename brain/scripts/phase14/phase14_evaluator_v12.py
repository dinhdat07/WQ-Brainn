...
def main():
    ...
    experiments = [
        # Hybrid 1: Distress + Kaku Order Flow (Combining our 1.50 and 1.60 alphas)
        {
            "name": "Hybrid_Distress_Kaku",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Hybrid 2: Cashflow + Kaku Order Flow
        {
            "name": "Hybrid_Cashflow_Kaku",
            "code": "ts_decay_linear(group_rank(cashflow_op / (assets + 1), subindustry) * rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Hybrid 3: Long-term Momentum + Reversal
        {
            "name": "Hybrid_Mom_Reversal",
            "code": "ts_decay_linear(rank(ts_delta(close, 252)) * rank(-ts_delta(close, 5)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Hybrid 4: Momentum + Kaku Order Flow (Buy strong companies when they crash into close)
        {
            "name": "Hybrid_Mom_Kaku",
            "code": "ts_decay_linear(rank(ts_delta(close, 252)) * rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F8: Sales/EV + Kaku Order Flow (Value + Microstructure)
        {
            "name": "Hybrid_Value_Kaku",
            "code": "ts_decay_linear(group_rank(sales / (market_cap + debt + 1), subindustry) * rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V12 EXPERIMENT RESULTS:")
    for r in results:
