...
def main():
    ...
    experiments = [
        # Push F1 Distress Reversal over the SubUniverse Sharpe hurdle (0.64) by filtering Panic Selling
        {
            "name": "F1_Distress_Panic_Volume",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * group_rank(-ts_delta(close, 5), subindustry) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # New Factor Family: Short Squeeze Reversal
        {
            "name": "F4_Short_Squeeze_Reversal",
            "code": "ts_decay_linear(group_rank(short_interest / (shares_out + 1), subindustry) * group_rank(-ts_delta(close, 5), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Kakushadze 101 Intraday Trend
        {
            "name": "F2_Kakushadze_101",
            "code": "ts_decay_linear(rank(close - open) / rank(high - low + 0.001), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Testing exponential decay on our best model (Sharpe 1.60)
        {
            "name": "F2_Kaku_Exp_Decay",
            "code": "ts_decay_exp(rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V10 EXPERIMENT RESULTS:")
    for r in results:
