...
def main():
    ...
    experiments = [
        # Framework 2 refinement: Higher decay and/or z-score
        {
            "name": "F2_OrderFlow_zscore",
            "code": "ts_decay_linear(group_zscore((vwap - close) / close, subindustry) * group_zscore(volume / adv20, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_OrderFlow_rank15",
            "code": "ts_decay_linear(group_rank((vwap - close) / close, subindustry) * group_rank(volume / adv20, subindustry), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        },
        
        # Framework 1 refinement: Pure Value/Credit Z-Score
        {
            "name": "F1_Credit_pure",
            "code": "ts_decay_linear(group_zscore(working_capital / (debt + 1), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F1_Credit_pure2",
            "code": "ts_decay_linear(group_zscore((working_capital - debt) / assets, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Framework 3 refinement: Target Price divergence
        {
            "name": "F3_Analyst_divergence",
            "code": "ts_decay_linear(rank(-ts_corr(est_ptp, close, 20)), 5)",
            "decay": 5,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F3_Analyst_divergence2",
            "code": "ts_decay_linear(rank(-ts_corr(est_fcf, close, 20)), 5)",
            "decay": 5,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V3 EXPERIMENT RESULTS:")
    for r in results:
