...
def main():
    ...
    experiments = [
        # Optimize F2 (Order Flow Imbalance) -> Aim Sharpe > 2.5
        {
            "name": "F2_Kakushadze_Mod",
            "code": "ts_decay_linear(rank(vwap - close) / rank(vwap + close), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Kakushadze_Vol",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / (adv20 + 1)), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_OrderFlow_ZScore",
            "code": "ts_decay_linear(group_zscore((vwap - close) / (high - low + 0.001), subindustry) * group_zscore(volume / adv20, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Optimize F1 (Credit Stress)
        {
            "name": "F1_Distress_ZScore",
            "code": "ts_decay_linear(group_zscore((debt - working_capital) / (assets + 1), subindustry) * group_zscore(-ts_delta(close, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F1_Distress_ZScore_Long",
            "code": "ts_decay_linear(group_zscore((debt - working_capital) / (assets + 1), subindustry) * group_zscore(-ts_delta(close, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        
        # Optimize F3 (Analyst Divergence)
        {
            "name": "F3_Analyst_Div_Rank",
            "code": "ts_decay_linear(group_rank(ts_corr(est_ptp, close, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V5 EXPERIMENT RESULTS:")
    for r in results:
