...
def main():
    ...
    experiments = [
        # F2 (Order Flow Imbalance) -> Kakushadze variant V6
        {
            "name": "F2_OrderFlow_Rankx3",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * rank(high - low), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_OrderFlow_GroupRankx2",
            "code": "ts_decay_linear(group_rank((vwap - close) / (high - low + 0.001), subindustry) * group_rank(volume / adv20, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F1 (Credit Stress) -> using Group Rank for proper combination
        {
            "name": "F1_Distress_Reversal_V6",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * group_rank(-ts_delta(close, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F1_Distress_Only_Rank",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        
        # F3 (Analyst Divergence) -> Combined with Reversal (using Ranks!)
        {
            "name": "F3_Analyst_Div_Combo",
            "code": "ts_decay_linear(group_rank(ts_corr(est_ptp, close, 20), subindustry) * group_rank(-ts_delta(close, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F3_Analyst_Div_Combo2",
            "code": "ts_decay_linear(rank(ts_corr(est_ptp, close, 40)) * rank(-ts_delta(close, 20)), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V6 EXPERIMENT RESULTS:")
    for r in results:
