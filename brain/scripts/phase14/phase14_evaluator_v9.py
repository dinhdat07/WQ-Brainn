...
def main():
    ...
    experiments = [
        # Push F1 Distress Reversal over the SubUniverse Sharpe hurdle (0.64)
        {
            "name": "F1_Distress_Sector",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), sector) * group_rank(-ts_delta(close, 5), sector), 10)",
            "decay": 10,
            "neut": "SECTOR"
        },
        {
            "name": "F1_Distress_Smooth15",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * group_rank(-ts_delta(close, 5), subindustry), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        },
        
        # Supercharge F2 Kakushadze Order Flow (> 2.0 Sharpe Target)
        {
            "name": "F2_Kakushadze_Liquidity",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * rank(adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Kakushadze_Volatility",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * rank(stddev(returns, 20)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Kakushadze_Intraday",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * rank(high - low), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F3 Analyst Divergence Fix (Handle NAs)
        {
            "name": "F3_Analyst_Upside_Fix",
            "code": "ts_decay_linear(group_rank((ts_backfill(est_ptp, 60) - close)/close, subindustry) * group_rank(-ts_delta(close, 5), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V9 EXPERIMENT RESULTS:")
    for r in results:
