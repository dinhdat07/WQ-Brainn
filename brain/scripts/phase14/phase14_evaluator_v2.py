...
def main():
    ...
    experiments = [
        # Framework 1: Credit Stress (Deleveraging + Momentum)
        {
            "name": "F1_Credit_v2",
            "code": "ts_decay_linear(group_zscore(working_capital / (debt + 1), subindustry) * ts_rank(-ts_delta(close, 20), 20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F1_Credit_v3",
            "code": "ts_decay_linear(group_rank((working_capital - debt) / assets, subindustry) * ts_rank(close / vwap, 10), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        },
        
        # Framework 2: Order Flow Imbalance (Smoothed)
        {
            "name": "F2_OrderFlow_v2",
            "code": "ts_decay_linear(ts_rank(ts_mean(vwap - close, 5) / close, 10) * ts_rank(ts_mean(volume, 5) / adv20, 10), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_OrderFlow_v3",
            "code": "ts_decay_linear(ts_rank((vwap - close) / close, 20) * ts_rank(volume / adv20, 20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        
        # Framework 3: Analyst Contrarian (Target Price Divergence)
        {
            "name": "F3_Analyst_v2",
            "code": "ts_decay_linear(ts_rank(ts_delta(est_ptp, 20) / est_ptp, 20) * ts_rank(-ts_delta(close, 20) / close, 20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F3_Analyst_v3",
            "code": "ts_decay_linear(group_zscore(ts_delta(est_fcf, 20) / est_fcf, subindustry) * group_zscore(-ts_delta(close, 20) / close, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V2 EXPERIMENT RESULTS:")
    for r in results:
        print(f"[{r['alpha_id']}] {r['name']} | Sh={r['sharpe']:.2f} | Fit={r['fitness']:.2f} | TO={r['turnover']*100:.1f}% | Corr={r['max_corr']:.4f} vs {r['max_corr_id']} | Fails={r['failed_checks']}")
