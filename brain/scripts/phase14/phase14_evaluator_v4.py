...
def get_daily_pnl_returns(session, alpha_id):
    ...
    for i in range(1, len(records)):
        dt = records[i][0][:10] # Ensure YYYY-MM-DD
...
def main():
    ...
    print(f"\nKeys for first active OS alpha: {list(active_rets.values())[0].keys() if active_rets else 'N/A'}")
    
    experiments = [
        # F1: Distress Reversal
        {
            "name": "F1_Distress_Reversal",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * ts_rank(-ts_delta(close, 5), 5), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        # F1: Distress Trend
        {
            "name": "F1_Distress_Trend",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * ts_rank(ts_delta(close, 60), 60), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        ...
        # F2: Kakushadze Style
        {
            "name": "F2_Kakushadze_Style",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        # F2: OrderFlow Rank20
        {
            "name": "F2_OrderFlow_rank20",
            "code": "ts_decay_linear(group_rank((vwap - close) / close, subindustry) * group_rank(volume / adv20, subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        ...
        # F3: Analyst div pos
        {
            "name": "F3_Analyst_div_pos",
            "code": "ts_decay_linear(rank(ts_corr(est_ptp, close, 20)), 5)",
            "decay": 5,
            "neut": "SUBINDUSTRY"
        },
        # F3: Analyst div pos smooth
        {
            "name": "F3_Analyst_div_pos_smooth",
            "code": "ts_decay_linear(rank(ts_corr(est_ptp, close, 40)), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V4 EXPERIMENT RESULTS:")
    for r in results:
