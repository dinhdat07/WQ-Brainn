...
def main():
    ...
    experiments = [
        # Optimize Kakushadze Decay 5 (Faster reaction)
        {
            "name": "F2_Kaku_Decay_5",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20), 5)",
            "decay": 5,
            "neut": "SUBINDUSTRY"
        },
        
        # Optimize Kakushadze Decay 15 (Slower reaction)
        {
            "name": "F2_Kaku_Decay_15",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        },
        
        # Time-Series Z-Score Reversion
        {
            "name": "F7_TS_Zscore_Rev_5d",
            "code": "ts_decay_linear(group_rank(-ts_zscore(close, 5), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Time-Series Z-Score Reversion + Volume Panic
        {
            "name": "F7_TS_Zscore_Panic",
            "code": "ts_decay_linear(group_rank(-ts_zscore(close, 5), subindustry) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Kakushadze + TS Zscore
        {
            "name": "F7_Kaku_Zscore",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * group_rank(-ts_zscore(close, 10), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V15 EXPERIMENT RESULTS:")
    for r in results:
