...
def main():
    ...
    experiments = [
        # Triad 1: Cashflow + IV Skew 60d + Kakushadze Vwap
        {
            "name": "F8_Triad_Cashflow_IV_Vwap",
            "code": "ts_decay_linear(group_rank(cashflow_op / (assets + 1), subindustry) + group_rank(trade_when(ts_backfill(pcr_oi_60, 60) < 1, (ts_backfill(implied_volatility_call_60, 60) - ts_backfill(implied_volatility_put_60, 60)), -1), subindustry) + group_rank(vwap - close, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Triad 2: Distress + Put Skew + Kakushadze Vwap
        {
            "name": "F8_Triad_Distress_IV_Vwap",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) + group_rank(trade_when(ts_backfill(pcr_oi_20, 60) > 1, (ts_backfill(implied_volatility_put_20, 60) - ts_backfill(implied_volatility_call_20, 60)), -1), subindustry) + group_rank(vwap - close, subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Triad 3: Sales/EV + IV Skew + Panic Volume
        {
            "name": "F8_Triad_SalesEV_IV_Panic",
            "code": "ts_decay_linear(group_rank(sales / (cap + debt + 1), subindustry) + group_rank(trade_when(ts_backfill(pcr_oi_270, 60) < 1, (ts_backfill(implied_volatility_call_270, 60) - ts_backfill(implied_volatility_put_270, 60)), -1), subindustry) + rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Hybrid: Kakushadze Vwap * IV Skew (Multiplicative like Phase 12 Spectacular)
        {
            "name": "F9_Kaku_IV_Hybrid",
            "code": "ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V17 EXPERIMENT RESULTS:")
    for r in results:
