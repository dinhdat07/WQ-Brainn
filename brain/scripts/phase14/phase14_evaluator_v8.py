...
def main():
    ...
    experiments = [
        # SUPERCHARGE F2: Intraday Reversal + Trend / Multi-Reversal
        {
            "name": "F2_Triple_Reversal",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * rank(-ts_delta(close, 5)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Reversal_in_Uptrend",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * rank(ts_delta(close, 20)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # SUPERCHARGE F3: Analyst Upside Reversal
        {
            "name": "F3_Analyst_Upside_Reversal_5d",
            "code": "ts_decay_linear(group_rank((est_ptp - close)/close, subindustry) * group_rank(-ts_delta(close, 5), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F3_Analyst_Upside_Reversal_20d",
            "code": "ts_decay_linear(group_rank((est_ptp - close)/close, subindustry) * group_rank(-ts_delta(close, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # SUPERCHARGE F1: Quality Reversal (FCF to Assets + Reversal)
        {
            "name": "F1_Quality_Reversal",
            "code": "ts_decay_linear(group_rank((cashflow_op - capital_expend) / (assets + 1), subindustry) * group_rank(-ts_delta(close, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # SUPERCHARGE F1: Extreme Distress Reversal 5d
        {
            "name": "F1_Distress_Reversal_5d",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * group_rank(-ts_delta(close, 5), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V8 EXPERIMENT RESULTS:")
    for r in results:
