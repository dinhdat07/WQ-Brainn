...
def main():
    ...
    experiments = [
        # Zero-Masking: Extreme Volume Spike (200% of 20-day average)
        {
            "name": "F2_Mask_Vol2x",
            "code": "ts_decay_linear(rank(vwap - close) * (volume > adv20 * 2.0 ? 1 : 0), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Zero-Masking: Gap Down (>2%)
        {
            "name": "F2_Mask_GapDown",
            "code": "ts_decay_linear(rank(vwap - close) * (open < ts_delay(close, 1) * 0.98 ? 1 : 0), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Zero-Masking: 5-Day Crash (>10%)
        {
            "name": "F2_Mask_Crash10",
            "code": "ts_decay_linear(rank(vwap - close) * (close < ts_delay(close, 5) * 0.90 ? 1 : 0), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Zero-Masking: Distressed Companies Only (Debt > 1.5x Working Capital)
        {
            "name": "F2_Mask_Distress",
            "code": "ts_decay_linear(rank(vwap - close) * (debt > working_capital * 1.5 ? 1 : 0), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Zero-Masking: Combined Volume + Gap Down
        {
            "name": "F2_Mask_Combined",
            "code": "ts_decay_linear(rank(vwap - close) * (volume > adv20 * 1.5 ? 1 : 0) * (close < ts_delay(close, 1) * 0.98 ? 1 : 0), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V16 EXPERIMENT RESULTS:")
    for r in results:
