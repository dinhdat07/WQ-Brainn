...
def main():
    ...
    experiments = [
        # Accumulation Distribution Line (ADL) with Panic Volume
        {
            "name": "F2_ADL_Panic",
            "code": "ts_decay_linear(rank(((close - low) - (high - close)) / (high - low + 0.001)) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        ...
        {
            "name": "F2_Overnight_Intraday",
            "code": "ts_decay_linear(rank(ts_delay(close, 1) - open) * rank(close - open), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        ...
        {
            "name": "F2_Kaku_Alpha6",
            "code": "ts_decay_linear(rank(-1 * correlation(open, volume, 10)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        ...
        # Kakushadze Alpha 12
        {
            "name": "F2_Kaku_Alpha12",
            "code": "ts_decay_linear(rank(sign(ts_delta(volume, 1)) * (-1 * ts_delta(close, 1))), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Today's Return Reversal with Panic Volume
        {
            "name": "F2_Panic_Returns",
            "code": "ts_decay_linear(rank(-returns) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V14 EXPERIMENT RESULTS:")
    for r in results:
