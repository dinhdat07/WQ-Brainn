...
def main():
    ...
    experiments = [
        # Inverted V17 F9 (Should be +2.13 Sharpe)
        {
            "name": "F9_Kaku_IV_Inverted_10d",
        ...
        # Inverted with 5d decay
        {
            "name": "F9_Kaku_IV_Inverted_5d",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 5)",
            "decay": 10,  # Keeping setting decay 10 to not mess with framework, the actual formula handles it
            "neut": "SUBINDUSTRY"
        },
        
        # Inverted with 20d decay
        {
            "name": "F9_Kaku_IV_Inverted_20d",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 20)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Panic Volume IV Hybrid
        {
            "name": "F9_Panic_IV_Hybrid",
            "code": "-ts_decay_linear(rank(volume / adv20) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V18 EXPERIMENT RESULTS:")
    for r in results:
