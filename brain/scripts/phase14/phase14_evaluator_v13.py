...
def main():
    ...
    experiments = [
        # Opt 1: Smoothed Kaku (Applying short-term moving average before ranking)
        {
            "name": "F2_Kaku_Smoothed",
            "code": "ts_decay_linear(rank(ts_mean(vwap - close, 3)) * rank(ts_mean(volume / adv20, 3)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Opt 2: Sector-Neutral Kaku
        {
            "name": "F2_Kaku_Sector_Neutral",
            "code": "ts_decay_linear(group_rank(vwap - close, sector) * group_rank(volume / adv20, sector), 10)",
            "decay": 10,
            "neut": "SECTOR"
        },
        
        # Opt 3: Pure Kakushadze Alpha 41 Adaptation
        {
            "name": "F2_Kaku_Alpha41",
            "code": "ts_decay_linear(rank((high * low)^0.5 - vwap), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Opt 4: Kakushadze Alpha 54 Adaptation
        {
            "name": "F2_Kaku_Alpha54",
            "code": "ts_decay_linear(rank(-1 * (low - close) / (high - low + 0.0001)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # Opt 5: Kaku with Volatility Scaling
        {
            "name": "F2_Kaku_Vol_Scaled",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20) / rank(ts_std_dev(returns, 20)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V13 EXPERIMENT RESULTS:")
    for r in results:
