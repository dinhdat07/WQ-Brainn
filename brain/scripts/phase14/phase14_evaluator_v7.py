...
def main():
    ...
    experiments = [
        # F2 (Order Flow Imbalance) -> Kakushadze variant V4 Re-Test
        {
            "name": "F2_Kakushadze_Style_Decay10",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Kakushadze_Style_Decay20",
            "code": "ts_decay_linear(rank(vwap - close) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Kakushadze_Style_Vol_Filter",
            "code": "ts_decay_linear(rank((vwap - close)/close) * rank(volume / adv20), 15)",
            "decay": 15,
            "neut": "SUBINDUSTRY"
        },
        
        # F1 (Credit Stress) -> Momentum instead of Reversal
        {
            "name": "F1_Distress_Trend",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * group_rank(ts_delta(close, 60), subindustry), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F3 (Analyst Divergence) -> Buy correlated and trending UP
        {
            "name": "F3_Analyst_Div_Trend",
    ...
    print("\n=======================================================")
    print("PHASE 14 V7 EXPERIMENT RESULTS:")
    for r in results:
