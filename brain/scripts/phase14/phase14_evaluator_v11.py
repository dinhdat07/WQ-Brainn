...
def main():
    ...
    experiments = [
        # F2 Kakushadze Alpha 101 Intraday Momentum
        {
            "name": "F2_Kakushadze_101",
            "code": "ts_decay_linear(rank((close - open) / (high - low + 0.001)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F2 Kakushadze 41: Midpoint deviation
        {
            "name": "F2_Kakushadze_41",
            "code": "ts_decay_linear(rank((high * low)^0.5 - vwap) * rank(volume / adv20), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F5 Price Reversal + Volume Trend
        {
            "name": "F5_Price_Vol_Reversal",
            "code": "ts_decay_linear(rank(-ts_delta(close, 5)) * rank(ts_delta(volume, 5)), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        
        # F6 Volatility Continuation
        {
            "name": "F6_Vol_Continuation",
            "code": "ts_decay_linear(rank(ts_delta(close, 20)) * rank(high - low), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        },
        ...
        # F1 Supercharger
        {
            "name": "F1_Distress_Intraday_Drop",
            "code": "ts_decay_linear(group_rank((debt - working_capital) / (assets + 1), subindustry) * rank(open - close), 10)",
            "decay": 10,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 14 V11 EXPERIMENT RESULTS:")
    for r in results:
