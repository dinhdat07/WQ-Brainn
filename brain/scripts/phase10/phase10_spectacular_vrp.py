"""
Phase 10: Spectacular Triad VRP + Alpha41 + Fundamental
Targeting Sharpe > 2.5 and strictly < 0.70 correlation with previous models.
"""
...
def main():
    ...
    variations = [
        # Test 1: Alpha 41 pure
        {
            "name": "Alpha 41 Reversion Pure",
            "expr": "ts_decay_linear(group_rank(((high * low)^0.5) - vwap, subindustry), 10)",
            "decay": 0, "trunc": 0.08, "neut": "SUBINDUSTRY"
        },
        # Test 2: VRP + Alpha 41
        {
            "name": "VRP + Alpha 41",
            "expr": "ts_decay_linear(group_rank(ts_backfill(implied_volatility_mean_120, 60) / ts_backfill(parkinson_volatility_120, 60), subindustry) + group_rank(((high * low)^0.5) - vwap, subindustry), 10)",
            "decay": 0, "trunc": 0.08, "neut": "SUBINDUSTRY"
        },
        # Test 3: The Triad
        {
            "name": "Triad: Sales + VRP + Alpha41",
            "expr": "ts_decay_linear(group_rank(sales / assets, subindustry) + group_rank(ts_backfill(implied_volatility_mean_120, 60) / ts_backfill(parkinson_volatility_120, 60), subindustry) + group_rank(((high * low)^0.5) - vwap, subindustry), 15)",
            "decay": 0, "trunc": 0.08, "neut": "SUBINDUSTRY"
        }
    ]
