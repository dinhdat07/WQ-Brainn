...
def main():
    ...
    experiments = [
        {
            "name": "F8_Mega_Orthogonal_Catalyst",
            "code": "ts_decay_linear(rank(vwap - close) * (group_rank(capex / (assets+1), subindustry) + group_rank(ts_backfill(snt_social_value, 10), subindustry) + group_rank(ts_zscore(est_eps, 250), subindustry)), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY",
            "universe": "TOP1000"
        },
        {
            "name": "F8_Mega_Orthogonal_Catalyst_Inv",
            "code": "-ts_decay_linear(rank(vwap - close) * (group_rank(capex / (assets+1), subindustry) + group_rank(ts_backfill(snt_social_value, 10), subindustry) + group_rank(ts_zscore(est_eps, 250), subindustry)), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY",
            "universe": "TOP1000"
        ...
        {
            "name": "F8_ShortInterest_Direct",
            "code": "ts_decay_linear(rank(vwap - close) * group_rank(shares_sold_short, subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F8_ShortInterest_Direct_Inv",
            "code": "-ts_decay_linear(rank(vwap - close) * group_rank(shares_sold_short, subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F8_Fundamental_Capex_Sales_VolMask",
            "code": "ts_decay_linear((group_rank(ts_zscore(ts_backfill(capex, 60) / (assets+1), 250), subindustry) + group_rank(ts_zscore(est_sales, 250), subindustry)) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 8 EXPERIMENT RESULTS:")
    for r in results:
