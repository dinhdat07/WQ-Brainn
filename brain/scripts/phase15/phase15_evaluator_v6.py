...
def main():
    ...
    experiments = [
        {
            "name": "F6_Analyst_Revisions",
            "code": "ts_decay_linear(rank(ts_delta(est_eps, 20)) * group_rank(ts_zscore(est_eps, 250), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F6_Sales_Revision_Momentum",
            "code": "ts_decay_linear(group_rank(ts_delta(est_sales, 20) / close, subindustry) * rank(close / ts_mean(close, 20)), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F6_Short_Squeeze",
            "code": "ts_decay_linear(group_rank(ts_backfill(si_short_interest_ratio, 20), subindustry) * rank(vwap - close), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F6_Short_Squeeze_Vol_Adjusted",
            "code": "-ts_decay_linear(group_rank(ts_backfill(si_short_interest, 20) / (volume + 1), subindustry) * rank(close - vwap), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 6 EXPERIMENT RESULTS:")
    for r in results:
