...
def main():
    ...
    experiments = [
        {
            "name": "F13_ShortSqueeze_Mean",
            "code": "ts_decay_linear(rank(vwap - close) * group_zscore(ts_mean(shares_sold_short, 60), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F13_ShortSqueeze_Mean_Inv",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_mean(shares_sold_short, 60), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F13_PriceVol_Crash",
            "code": "-ts_decay_linear(rank(vwap - close) * rank(ts_std_dev(returns, 60)), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F13_Fundamental_Capex_Sales_VolMask_Aggressive",
            "code": "ts_decay_linear((group_rank(ts_zscore(ts_mean(capex, 60) / (assets+1), 250), subindustry) + group_rank(ts_zscore(ts_mean(est_sales, 60), 250), subindustry)) * rank(volume / adv20), 5)",
            "decay": 5,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 13 EXPERIMENT RESULTS:")
    for r in results:
