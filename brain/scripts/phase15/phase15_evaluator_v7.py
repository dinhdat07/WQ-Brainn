...
def main():
    ...
    experiments = [
        {
            "name": "F7_Skew120_Inverted_Spectacular",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_120 - implied_volatility_call_120, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F7_Skew60_Inverted_Spectacular",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_60 - implied_volatility_call_60, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F7_Skew20_Momentum_Shift",
            "code": "-ts_decay_linear(rank(close / ts_mean(close, 20)) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F7_Short_Interest_Squeeze",
            "code": "ts_decay_linear(group_rank(ts_backfill(shares_sold_short, 20), subindustry) * rank(vwap - close), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 7 EXPERIMENT RESULTS:")
    for r in results:
