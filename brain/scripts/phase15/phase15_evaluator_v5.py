...
def main():
    ...
    experiments = [
        {
            "name": "F5_Skew_Reversal_Capex",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry) * group_rank(capex / (assets+1), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F5_Skew_Reversal_Volume",
            "code": "-ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 40)",
            "decay": 40,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F5_Skew_MeanRev_Triple",
            "code": "-ts_decay_linear(rank(vwap - close) * rank(ts_zscore(implied_volatility_put_20 - implied_volatility_call_20, 20)) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F5_Social_MeanRev_Triple",
            "code": "-ts_decay_linear(rank(vwap - close) * group_rank(ts_backfill(snt_social_value, 10), subindustry) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY",
            "universe": "TOP1000"
        }
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 5 EXPERIMENT RESULTS:")
    for r in results:
