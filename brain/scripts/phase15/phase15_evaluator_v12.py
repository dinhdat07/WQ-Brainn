...
def main():
    ...
    experiments = [
        {
            "name": "F12_Skew_Sector_Neut",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), sector), 20)",
            "decay": 20,
            "neut": "SECTOR"
        },
        {
            "name": "F12_Skew_Market_Neut",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), market), 20)",
            "decay": 20,
            "neut": "MARKET"
        },
        {
            "name": "F12_Skew_LowClose_Reversion",
            "code": "-ts_decay_linear(rank(low - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F12_Skew_Return_Reversion",
            "code": "-ts_decay_linear(rank(-returns) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 12 EXPERIMENT RESULTS:")
    for r in results:
