...
def main():
    ...
    experiments = [
        {
            "name": "F2_Accel_RD_Capex_Intraday",
            "code": "ts_decay_linear(group_rank(ts_zscore(ts_backfill(rd_expense, 60) / (assets + 1), 250), subindustry) + group_rank(ts_zscore(ts_backfill(capex, 60) / (assets + 1), 250), subindustry) + group_rank(-(close - open) / open, subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Accel_RD_Only",
            "code": "ts_decay_linear(group_rank(ts_zscore(ts_backfill(rd_expense, 60) / (assets + 1), 250), subindustry) + group_rank(-(close - open) / open, subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Accel_Capex_Only",
            "code": "ts_decay_linear(group_rank(ts_zscore(ts_backfill(capex, 60) / (assets + 1), 250), subindustry) + group_rank(-(close - open) / open, subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F2_Accel_RD_Capex_VolMask",
            "code": "ts_decay_linear((group_rank(ts_zscore(ts_backfill(rd_expense, 60) / (assets + 1), 250), subindustry) + group_rank(ts_zscore(ts_backfill(capex, 60) / (assets + 1), 250), subindustry)) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 2 EXPERIMENT RESULTS:")
    for r in results:
