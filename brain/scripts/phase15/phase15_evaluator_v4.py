...
def main():
    ...
    experiments = [
        {
            "name": "F4_Hybrid_Skew_TermStruct_Inverted",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(implied_volatility_put_20 - implied_volatility_call_20, 20) + ts_backfill(implied_volatility_put_20 - implied_volatility_put_270, 20), subindustry) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F4_Social_Vol_Reversal",
            "code": "-ts_decay_linear(rank(vwap - close) * rank(volume / adv20) * ts_backfill(snt_social_value, 10), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY",
            "universe": "TOP1000"
        },
        {
            "name": "F4_Hybrid_Skew_TermStruct_Pure",
            "code": "-ts_decay_linear(group_rank(ts_backfill(implied_volatility_put_20 - implied_volatility_call_270, 20), subindustry) * rank(vwap - close) * rank(volume / adv20), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 4 EXPERIMENT RESULTS:")
    for r in results:
