...
def main():
    ...
    experiments = [
        {
            "name": "F9_Parkinson_Vol_Crash",
            "code": "-ts_decay_linear(rank(vwap - close) * group_zscore(ts_backfill(parkinson_volatility_20, 20), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F9_Insider_Crash",
            "code": "-ts_decay_linear(rank(vwap - close) * group_rank(ts_backfill(rp_css_insider, 5), subindustry), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        },
        {
            "name": "F9_Fundamental_Momentum_Pro",
            "code": "ts_decay_linear((group_rank(ts_zscore(ts_backfill(capex, 60) / (assets+1), 250), subindustry) + group_rank(ts_zscore(est_sales, 250), subindustry)) * rank(close / ts_mean(close, 250)), 20)",
            "decay": 20,
            "neut": "SUBINDUSTRY"
        }
    ]
    ...
    print("\n=======================================================")
    print("PHASE 15 BATCH 9 EXPERIMENT RESULTS:")
    for r in results:
