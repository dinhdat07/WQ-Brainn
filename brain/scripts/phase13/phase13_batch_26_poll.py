...
def main():
    ...
    res_file = r"E:\CODING\MMO\wq-brain\WQ-Brainn\brain\scripts\phase13\phase13_batch_26_results.json"
    with open(res_file, "r") as f:
        alphas = json.load(f)
        
    pending = [n for n, d in alphas.items() if d.get("status") == "simulating"]
    print(f"Polling {len(pending)} pending simulations for Batch 26...")
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if st in ["ERROR"]:
                        ...
                        alphas[n]["error"] = err
                        pending.remove(n)
                    elif st in ["COMPLETE", "WARNING"]:
                        alpha_id = resp.json().get("alpha")
                        alphas[n]["status"] = st
                        alphas[n]["alpha_id"] = alpha_id
                        print(f"[{n}] {st}! Alpha ID: {alpha_id}")
                        pending.remove(n)
    ...
    for n, d in alphas.items():
        if d.get("status") in ["COMPLETE", "WARNING"] and d.get("alpha_id"):
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"\n[{n}] Alpha ID: {alpha_id}")
                    print(f"  Sharpe: {sharpe} | Fit: {fitness} | TO: {turnover*100:.1f}% | Ret: {ret*100:.1f}% | Margin: {margin*10000:.1f} bps | SubSh: {sub_sharpe}")
