...
def main():
    ...
    experiments = [
        # Track 1: Operating Income Yield + Working Capital Anomaly
        {
            "name": "Track 1: Operating Income Yield + Working Capital Efficiency",
            "expr": "ts_decay_linear(group_rank(operating_income / cap, subindustry) + group_rank(working_capital / assets, subindustry), 15)",
            "decay": 15
        },
        # Track 2: Forward EBIT Consensus Yield + Revision Momentum
        {
            "name": "Track 2: Forward EBIT Yield + 20d Consensus EBIT Revision",
            "expr": "ts_decay_linear(group_rank(anl4_ebit_mean / cap, subindustry) + group_rank(ts_delta(anl4_ebit_mean, 20) / (abs(anl4_ebit_mean) + 1), subindustry), 15)",
            "decay": 15
        },
        # Track 3: Analyst Recommendation Score + Forward EBIT
        {
            "name": "Track 3: Consensus Analyst Mark + Operating Income Yield",
            "expr": "ts_decay_linear(group_rank(ts_backfill(anl4_mark, 20), subindustry) + group_rank(operating_income / cap, subindustry), 15)",
            "decay": 15
        },
        # Track 4: Alpha 101 #41 Geometric High-Low Spread vs VWAP + Operating Income Yield
        {
            "name": "Track 4: Alpha 101 #41 (High-Low VWAP Spread) + Operating Income Yield",
            "expr": "ts_decay_linear(group_rank(power(high * low, 0.5) - vwap, subindustry) + group_rank(operating_income / cap, subindustry), 12)",
            "decay": 12
        },
        # Track 5: Alpha 101 #101 Normalized Candlestick + Free Cash Flow Yield
        {
            "name": "Track 5: Alpha 101 #101 + Free Cash Flow Yield",
            "expr": "ts_decay_linear(group_rank(-(close - open) / ((high - low) + 0.001), subindustry) + group_rank(free_cash_flow_reported_value / cap, subindustry), 10)",
            "decay": 10
        },
        # Track 6: Volatility Term Structure (Mean 30d vs Mean 360d) + Operating Income
        {
            "name": "Track 6: Vol Term Structure Slope (30d vs 360d) + Operating Income Yield",
            "expr": "ts_decay_linear(group_rank((ts_backfill(implied_volatility_mean_30, 30) - ts_backfill(implied_volatility_mean_360, 30)) / ts_backfill(implied_volatility_mean_360, 30), subindustry) + group_rank(operating_income / cap, subindustry), 15)",
            "decay": 15
        },
        # Track 7: News Equity Sentiment Score + Operating Profitability
        {
            "name": "Track 7: Equity Sentiment Score + Operating Income Yield",
            "expr": "ts_decay_linear(group_rank(ts_backfill(equity_sentiment_score, 10), subindustry) + group_rank(operating_income / cap, subindustry), 10)",
            "decay": 10
        },
        # Track 8: 5-Day Multi-Horizon Mean Reversion + Working Capital
        {
            "name": "Track 8: 5-Day Return Reversal + Forward EBIT Yield",
            "expr": "ts_decay_linear(group_rank(-ts_delta(close, 5) / close, subindustry) + group_rank(anl4_ebit_mean / cap, subindustry), 12)",
            "decay": 12
        }
    ]

    print("\n================ STARTING ORTHOGONAL ALPHA HUNT V2 ================\n")
    results = []
    ...
    print("\n================ SUMMARY OF ALL ORTHOGONAL EXPERIMENTS V2 ================\n")
    for r in results:
