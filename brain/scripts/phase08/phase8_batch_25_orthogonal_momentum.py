...
def main():
    file_logger.info("Starting Phase 8.7 Batch 25 - Orthogonal Momentum & Volatility...")
    ...
    tasks = [
        ("25_1_AssetTurn_Mom252", "ts_decay_linear(group_zscore((sales / assets) * rank(ts_delta(close, 252)), subindustry), 5)", {}),
        ("25_2_GPA_Mom252", "ts_decay_linear(group_zscore(((sales - cogs) / assets) * rank(ts_delta(close, 252)), subindustry), 5)", {}),
        ("25_3_AssetTurn_VolArb", "ts_decay_linear(group_zscore((sales / assets) * rank(implied_volatility_call_120 / parkinson_volatility_120), subindustry), 5)", {}),
        ("25_4_GPA_VolArb", "ts_decay_linear(group_zscore(((sales - cogs) / assets) * rank(implied_volatility_call_120 / parkinson_volatility_120), subindustry), 5)", {}),
        ("25_5_AssetTurn_VolSurprise", "ts_decay_linear(group_zscore((sales / assets) * rank(ts_delta(volume, 5) * sign(returns)), subindustry), 5)", {})
    ]
