...
def main():
    file_logger.info("Starting Phase 8.8 Batch 26 - Fix Weight Concentration...")
    ...
    tasks = [
        ("26_1_AssetTurn_VolArb_Trunc05", "ts_decay_linear(group_zscore((sales / assets) * rank(implied_volatility_call_120 / parkinson_volatility_120), subindustry), 5)", {"truncation": 0.05}),
        ("26_2_AssetTurn_VolArb_Trunc03", "ts_decay_linear(group_zscore((sales / assets) * rank(implied_volatility_call_120 / parkinson_volatility_120), subindustry), 5)", {"truncation": 0.03}),
        ("26_3_AssetTurn_VolArb_Rank", "ts_decay_linear(group_rank((sales / assets) * rank(implied_volatility_call_120 / parkinson_volatility_120), subindustry), 5)", {"truncation": 0.08}),
        ("26_4_AssetTurn_VolArb_Decay10", "ts_decay_linear(group_zscore((sales / assets) * rank(implied_volatility_call_120 / parkinson_volatility_120), subindustry), 10)", {"truncation": 0.05})
    ]
