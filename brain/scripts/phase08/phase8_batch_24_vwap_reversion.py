...
def main():
    file_logger.info("Starting Phase 8 Batch 24 - VWAP and Medium Reversion (Decorrelation)...")
    ...
    tasks = [
        ("24_1_AssetTurn_VWAPClose", "ts_decay_linear(group_zscore((sales / assets) * rank(-(close - vwap)/vwap), subindustry), 5)", {}),
        ("24_2_AssetTurn_VWAPOpen", "ts_decay_linear(group_zscore((sales / assets) * rank(-(vwap - open)/open), subindustry), 5)", {}),
        ("24_3_GPA_VWAPClose", "ts_decay_linear(group_zscore(((sales - cogs) / assets) * rank(-(close - vwap)/vwap), subindustry), 5)", {}),
        ("24_4_GPA_Rev10d", "ts_decay_linear(group_zscore(((sales - cogs) / assets) * rank(-ts_delta(close, 10)), subindustry), 5)", {}),
        ("24_5_OpMargin_Rev20d", "ts_decay_linear(group_zscore((income / sales) * rank(-(close / ts_mean(close, 20))), subindustry), 5)", {})
    ]
