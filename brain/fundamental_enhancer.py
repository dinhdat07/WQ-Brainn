...
def main():
    logger.info("Starting FUNDAMENTAL ENHANCER Alpha Generator (Fundamental x Price Reversion)")
    sess, _ = sign_in('brain_credentials.txt')
    
    formulas = []
    
    # Fundamental Variables that performed well in Batch 8, 20, 24
    fundamental_vars = [
        "fnd6_cptmfmq_oibdpq",
        "actual_sales_value_quarterly"
    ]
    
    # Template: ts_decay_linear(group_rank(ts_rank(FUND_VAR, D1), subindustry) * -1 * ts_rank(ts_delta(close, D2), D3), D4)
    # This buys fundamentally strong companies that are experiencing short-term price dips
        ...
        for d1 in [3, 5]:
            for d2 in [5, 10]:
                for d3 in [3, 5]:
                    field = var
                    grp = "subindustry"
                    expr = f"ts_decay_linear(group_rank(ts_rank({field}, 60), {grp}) * ts_rank(-ts_delta(close, {d1}), {d2}), {d3})"
                    formulas.append(expr)
                        
                    # also try with sector
                    expr_sector = f"ts_decay_linear(group_rank(ts_rank({var}, 60), sector) * ts_rank(-ts_delta(close, {d1}), {d2}), {d3})"
                    formulas.append(expr_sector)

    total = len(formulas)
    logger.info(f"Generated {total} enhanced fundamental formulas. Executing batch...")
    ...
    settings = {
        ...
        "delay": 1,
        "decay": 0,
        "neutralization": "NONE", # the group_rank acts as partial neutralization
        "truncation": 0.08,
        "pasteurization": "ON",
        "unitHandling": "VERIFY",
        "nanHandling": "OFF",
    ...
    logger.info("🔥 TOP SUBMITTABLE ENHANCED ALPHAS (Sharpe > 1.25 & Fitness > 1.0) 🔥")
